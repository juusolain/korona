...
function prepCheckboxes() {
  ...
  let percentageBox = document.getElementById("percentageBox");
  // let predictBox = document.getElementById('predictBox')

  percentageBox.checked = useCasePercentage;
  ...
  percentageBox.onclick = (e) => {
    useCasePercentage = e.target.checked;
    updateCases();
  };
}
...
function updateCaseChart(
  labels = cachedCaseLabels,
  datasets = cachedCaseDatasets
) {
  ...
  if (caseChart) {
    ...
    caseChart.options.scales.yAxes[0].ticks.callback = actuallyUsePercentage
      ? addPercentageSign
      : function (v) {
          return v;
        };
          ...
          yAxes: [
            {
              ...
              ticks: {
                callback: actuallyUsePercentage
                  ? addPercentageSign
                  : function (v) {
                      return v;
                    },
...
function updateVaccinationChart(
  labels = cachedVaccineLabels,
  datasets = cachedVaccineDatasets
) {
  if (vaccinationChart) {
    vaccinationChart.data.labels = labels;
    vaccinationChart.data.datasets = datasets;
    vaccinationChart.options.scales.yAxes[0].type = log
      ? "logarithmic"
      : "linear";
